            ...
            Option("module", "For which module do you want a new feature")] string module)
        {
            ...
            else if(confirm_result.Result.Id == $"{ctx.User.Id}-{title_request.Id}-yes")
            {
                ...
                try
                {
                    ...
                    var msg = await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent($"New feature request: https://bugs.aitsys.dev/T{task.Identifier}").AddEmbed(etask.GetEmbed()));
                    ...
                    await end_request.ModifyAsync($"Request send.\nSee https://bugs.aitsys.dev/T{task.Identifier}");
                } catch(Exception ex)
                {
